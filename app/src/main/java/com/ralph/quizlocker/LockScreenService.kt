...
class LockScreenService : Service() {
    ...
    // 화면이 꺼질 때 브로드캐스트 메시지를 수신하는 리시버
    var receiver: ScreenOffReceiver? = null

    private val ANDROID_CHANNEL_ID = "com.ralph.quizlocker"
    ...
    // 서비스가 호출하는 클라이언트가 startService() 함수를 호출할 때마다 불리는 콜백함수
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        super.onStartCommand(intent, flags, startId)

        if(intent != null) {
            if(intent.action == null) {
                // 서비스가 최초 실행이 아닌 경우는 onCreate가 불리지 않을 수 있음
                // 이 경우 receiver가 null이면 새로 생성하고 등록한다.
                if(receiver == null) {
                    receiver = ScreenOffReceiver()
                    val filter = IntentFilter(Intent.ACTION_SCREEN_OFF)
                    registerReceiver(receiver, filter)
